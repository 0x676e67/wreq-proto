...
pub trait RequestConfigValue: Clone + 'static {
    /// The concrete value type associated with this config key.
    ...
    T: RequestConfigValue,
{
    ...
    #[inline]
    pub const fn new(v: Option<T::Value>) -> Self {
        RequestConfig(v)
    }

    /// Returns a reference to the inner value of this request-scoped configuration.
    #[inline]
    pub const fn as_ref(&self) -> Option<&T::Value> {
        self.0.as_ref()
    }
    ...
    #[inline]
    pub fn fetch<'a>(&'a self, ext: &'a Extensions) -> Option<&'a T::Value> {
    ...
    #[inline]
    pub fn store<'a>(&'a self, ext: &'a mut Extensions) -> &'a mut Option<T::Value> {
    ...
    #[inline]
    pub fn load(&mut self, ext: &mut Extensions) -> Option<&T::Value> {
    ...
    #[inline]
    pub fn get(ext: &Extensions) -> Option<&T::Value> {
    ...
    /// `Option<T::Value>`.
    #[inline]
    pub fn get_mut(ext: &mut Extensions) -> &mut Option<T::Value> {
    ...
    #[inline]
    pub fn remove(ext: &mut Extensions) -> Option<T::Value> {
