...
impl Socks5Stream<TcpStream> {
    ...
    {
        let sock =
            Self::execute_command(proxy, target, Authentication::None, Command::TorResolve).await?;
    ...
    {
        let sock =
            Self::execute_command(proxy, target, Authentication::None, Command::TorResolvePtr)
                .await?;
    ...
    S: AsyncSocket + Unpin,
{
    ...
    where
        T: IntoTargetAddr<'t>,
    {
        Self::execute_command_with_socket(socket, target, Authentication::None, Command::Connect)
            .await
    }
    ...
    where
        T: IntoTargetAddr<'t>,
    {
        let sock = Self::execute_command_with_socket(
            socket,
            target,
            Authentication::None,
            Command::TorResolve,
    ...
    /// to it.
    pub async fn tor_resolve_ptr_with_socket<'t, T>(
        socket: S,
        target: T,
    ...
    S: Stream<Item = Result<SocketAddr>> + Unpin,
{
    ...
    pub async fn execute_with_socket<T: AsyncSocket + Unpin>(
        &mut self,
        mut socket: T,
    ...
    async fn password_authentication_protocol<T: AsyncSocket + Unpin>(
        &mut self,
        tcp: &mut T,
    ) -> Result<()> {
    ...
    async fn receive_reply<T: AsyncSocket + Unpin>(
        &mut self,
        tcp: &mut T,
    ) -> Result<TargetAddr<'static>> {
    ...
    S: AsyncSocket + Unpin,
{
    ...
    where
        T: IntoTargetAddr<'t>,
    {
        Self::bind_with_auth_and_socket(
            Authentication::Password { username, password },
            socket,
            target,
        )
